<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="采购编号" prop="caigoubianhao">
            <el-input v-model="ruleForm.caigoubianhao" 
                placeholder="采购编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.caigoubianhao" label="采购编号" prop="caigoubianhao">
              <el-input v-model="ruleForm.caigoubianhao" 
                placeholder="采购编号" readonly></el-input>
          </el-form-item>
        </div>
...
export default {
  ...
  computed: {
    caigoujine:{
      get: function () {
        return 1*this.ruleForm.caigoushuliang*this.ruleForm.caigoujiage
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.gongyingshangmingcheng = true;
          ...
          if(o=='caigoubianhao'){
            this.ruleForm.caigoubianhao = obj[o];
	    this.ro.caigoubianhao = true;
            continue;
          }
      ...
      this.$http({
		...
		if(json.yuangongxingming!=''&&json.yuangongxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shangpinxinxi/shangpinbianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
    ...
    shangpinbianhaoChange () {
      this.$http({
        url: `follow/shangpinxinxi/shangpinbianhao?columnValue=`+ this.ruleForm.shangpinbianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shangpincaigou/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shangpincaigouCrossAddOrUpdateFlag = false;
                  this.parent.search();
